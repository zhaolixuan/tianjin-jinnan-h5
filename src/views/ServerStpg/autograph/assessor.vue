<template>
  <div class="details">
    <nav class="box">
      ...
      <div class="van-uploader">
        <div class="uploaderBox"
             v-if="!zpgresultImg.url"
             @click="handleClick('zpgresultImg')">
          <van-icon name="edit" />
          <p>签名</p>
        </div>
        ...
        <div class="imgBox"
             v-if="zpgresultImg.url">
          <img :src="zpgresultImg.url"
               alt="">
          <div class="van-uploader__preview-delete"
               @click="delectImg(zpgresultImg)">
    ...
    <nav class="box">
      ...
      <div class="van-uploader">
        <div class="uploaderBox"
             v-if="!fpgresultImg.url"
             @click="handleClick('fpgresultImg')">
          <van-icon name="edit" />
          <p>签名</p>
        </div>
        ...
        <div class="imgBox"
             v-if="fpgresultImg.url">
          <img :src="fpgresultImg.url"
               alt="">
          <div class="van-uploader__preview-delete"
               @click="delectImg(fpgresultImg)">
            <i class="van-icon van-icon-cross van-uploader__preview-delete-icon"></i>
          </div>
        </div>
      </div>
    ...
    onConfirm() {
      this.$refs['esign']
        ...
        .then((res) => {
          ...
          forms.append(
            'fileType',
            this.activeData == 'zpgresultImg' ? '2' : '3'
          )
          forms.append('idCard', this.idcardNumber)
          upload(forms).then((res) => {
            this.popupShow = false
            this[this.activeData] = res.data[0] // 得到了签字生成的base64图片
          })
        })
        .catch((err) => {
          console.log(err)
